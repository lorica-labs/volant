...
- A pull request body carries **no** `Signed-off-by` trailer: `gh pr create --fill` puts one there, delete it afterward. Check with `gh pr view --json body --jq .body | grep -c Signed-off-by`, which must print `0`. The squash still takes the trailer from the branch commit, so the merged commit gets exactly one either way; the `signoff` CI check reads commits, not the body, so it cannot catch a duplicate.
