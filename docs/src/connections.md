...
Escalation adds a link rather than replacing one: while a play runs, a host that escalates to
two users holds three links, one per target user plus the one for the account you log in as.
Only that last one survives the play. An escalated link closes when its host finishes the play
and is reopened by the next play that needs it, which is cheap because the agent it wants is
already cached for that user. What persistence is for is the connection to the host itself, and
that is the link that stays.
...
number of users settles at N plus `forks` links. With the usual `ulimit -n 1024` that runs out
past roughly 338 links, and `ssh` then stops starting: the hosts it happens to hit are reported
unreachable although nothing is wrong with them. Raise `ulimit -n` before a run that wide.
