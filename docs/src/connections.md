...
These are read per host, from the inventory or from any other variable source. A test compares
the names below against the ones the connection code reads, so the list cannot fall behind the
engine; what each one does is written by hand.
...
A host keeps its agent link for the whole run, across plays. Tasks travel over that one link
instead of opening a connection each, in batches that are one task wide unless
[batching](batching.md) is turned on. If a link dies between two plays, Volant reconnects once;
a second failure reports the host unreachable.
...
An escalated link goes back in front of every wait on the other hosts, and by default every task
is one, because the hosts of a play meet in front of each of them. A play of six escalated tasks
therefore opens six escalated agents, one after another, each with its own `sudo` probe.

[Batching](batching.md) changes that. With it on, a host carries its link past a task that needs
nothing from anybody else, and the same play opens one agent. A wait is then a task
the keyword table marks as a synchronisation point, a task whose text reads another host's
state, a point where an include or a handler adds steps to the play, or the end of the play. A
host that escalates to a second user opens a link for that user either way.
[ADR 0005](https://github.com/lorica-labs/volant/blob/main/docs/adr/0005-fork-permits-span-host-local-batches.md)
records the rule and the option it sits behind.
...
with them. Raise `ulimit -n` before a run that wide. Budget for the waits rather than for the
hosts. With batching off an escalated play closes and reopens its links at every task, so those
overlapping closes go on for the length of the play instead of happening now and again.
