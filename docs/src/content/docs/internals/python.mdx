...
Volant has its own version of seven of these plugins: `copy`, `dnf`, `fetch`, `package`, `service`, `template` and `unarchive` run on the controller as a short sequence of sub-tasks, each an ordinary Python module sent over the host's existing connection. [Action plugins](/reference/action-plugins/) describes each one. The modules whose plugin Volant does not have yet, such as `reboot` and `uri`, are not supported yet: Volant names them before the first connection.
