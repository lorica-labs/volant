...
A playbook stopped by the [pre-flight check](/playbooks/preflight/) exits with 4 before any connection, which is what Ansible uses for a playbook it cannot load. Cases whose code was measured on Ansible keep that code: 1 for a missing role, an unknown `meta` action or an unknown `notify` name.
