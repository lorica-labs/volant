...
- Resolving variables costs more than linearly in the size of the inventory. Sharing the inventory values took half of that cost away: a task reads one host's entry out of a map every host shares, instead of copying every host's variables, so `hostvars` is no longer what makes the curve bend. What sharing did not touch is the magic variables that name the whole inventory. `groups` and the play's live host lists are still written into every host's variables for every task, and they are what still grows faster than the inventory does. Measured on a debug build over twenty local tasks, before 0.1.0-alpha.6: 50 hosts 0.26 s, 100 hosts 0.71 s, 200 hosts 2.19 s. The engine has since become faster, so read those three as the shape of the curve rather than as timings you should see today.
