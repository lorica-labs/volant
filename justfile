...
ssh-test: agent-musl
    ...
    # The python module golden, here because this is the job that names the recorded
    # ansible-core in VOLANT_PYTHON, which turns the comparison's skip into a failure.
    cargo nextest run --workspace --no-tests=fail -E 'test(=a_python_module_returns_the_reference_s_own_keys)'
